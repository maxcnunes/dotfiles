...
local function checkRequirement(command, msg)
  if not utils.isExecutableAvailable(command) then
    utils.notify(string.format('Missing %s binary or not found in PATH - %s', command, msg), vim.log.levels.WARN, 'core.checks')
  end
end
...
if not utils.isNeovimVersionsatisfied(9) then
  utils.notify(string.format "This config probably won't work very well with Neovim < 0.9", vim.log.levels.WARN, 'core.checks')
end
